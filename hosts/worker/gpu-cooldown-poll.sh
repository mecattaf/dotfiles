...
# temperature (falling back to k10temp Tctl when junction is absent — still the
# live case on this worker, re-verified 2026-08-21: its amdgpu hwmon exposes only
# `edge`, so the k10temp Tctl rail is the one that actually runs), and on a
# SUSTAINED over-threshold reading fires LAYER 2 (the shed adapter). This layer
# never decides WHAT to do about the heat; the adapter owns that seam.
#
...
#   * cooldown_until — while now < this, a shed has been performed and the box is
#                  expected to be cooling: suppressed.
#
...
adapter="${COOLDOWN_ADAPTER:?COOLDOWN_ADAPTER must point at the shed adapter}"
...
if [ "$decision" = "trigger" ]; then
  if "$adapter" "$temp_c" "$sensor_kind" "$threshold"; then
    ...
    first_over=0
    echo "gpu-cooldown: TRIGGERED at ${temp_c}C (${sensor_kind}); GPU load shed — disarmed until temp<${rearm_c}C AND the cooldown window elapses (until epoch ${cooldown_until})"
  else
    echo "gpu-cooldown: adapter FAILED to shed (GPU HOT at ${temp_c}C); staying armed, retry next poll" >&2
  fi
fi
