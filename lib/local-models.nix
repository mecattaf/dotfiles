        ...
        config = {
          ...
          artifacts = mageArtifacts // {
            ...
            flashnext-fp8 = {
              kind = "model";
              maker = "Qwen";
              notes = "flashnext TP=2 lane: the vendor FP8 release (125B trunk + 51.2B hash-lookup embedding, 6B active, 262144 context), 185.6 GB REQUIRED IN FULL ON EACH TWIN — tensor-parallel shards compute, not the on-disk checkpoint. Declared as an artifact, not a deployment: vLLM serves it through the flashnext pair service (github.com/mecattaf/flashnext, host/fn-cluster-up.sh), never as a llama-swap row. The row keeps wanted metadata and the guarded explicit prune oracle aligned; activation and boot never borrow or delete it. Historical warning: the retired sync service deleted both freshly staged copies on 2026-08-29, costing a 185.6 GB per-node re-stage. Apache-2.0 weights, governed by the upstream model license.";
