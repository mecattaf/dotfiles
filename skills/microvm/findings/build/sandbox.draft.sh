...
# encode_workload <word...> -- base64 command-transport (ERA's trick). Encode
# arbitrary agent-written code host-side, decode in-guest, hand to bash --
# dodging every layer of shell quoting. Prints the in-guest argv one word per
# line for the caller's `mapfile`. printf %s is byte-faithful (echo mangles
# leading `-` and backslashes); the in-guest wrapper pipes decoded bytes into
# `bash -s` (no exec, no echo).
encode_workload() {
  local payload b64
  payload="$(printf '%s ' "$@")"
  payload="${payload% }"
...
build_birth_argv() {
  ...
  if [ "${#WORKLOAD_CMD[@]}" -gt 0 ]; then
    local -a _wl
    mapfile -t _wl < <(encode_workload "${WORKLOAD_CMD[@]}")
    argv+=("${_wl[@]}")
  fi
...
# [Expert A; workload transport reconciled to encode_workload]
# ============================================================================
verb_exec() {
  reset_opts
  ...
  while IFS= read -r id; do
    ...
    case "$status" in
      exited|dead|stopped)
        ...
        ;;
      *)
        # Healthy / transitional (running/created/paused): never time-killed.
        :
        ;;
    esac
