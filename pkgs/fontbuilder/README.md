...
| `--careful` | it preserves the PUA but stops the patcher redrawing cell-filling box and block art: U+2588 becomes `(-50,0,1250,1440)` instead of a box spanning x -12 to 1212, so full block would no longer fill the cell. The PUA problem is solved by stage S7b instead. |
