...
func (_q *TaskQuery) Clone() *TaskQuery {
	...
	return &TaskQuery{
		...
		withCalendar: _q.withCalendar.Clone(),
...
func (_q *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := _q.withCalendar; query != nil {
		if err := _q.loadCalendar(ctx, query, nodes, nil,
			func(n *Task, e *Calendar) { n.Edges.Calendar = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TaskQuery) loadCalendar(ctx context.Context, query *CalendarQuery, nodes []*Task, init func(*Task), assign func(*Task, *Calendar)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Task)
	for i := range nodes {
		if nodes[i].calendar_tasks == nil {
			continue
		}
		fk := *nodes[i].calendar_tasks
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "calendar_tasks" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
