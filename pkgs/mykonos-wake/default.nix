...
in
runCommand "mykonos-wake-0.1.0"
  {
    nativeBuildInputs = [ makeWrapper ];
    meta = {
      description = "Client Alexa wake, listening cue and bounded command transcription";
      license = lib.licenses.asl20;
      mainProgram = "mykonos-wake";
    };
  }
  ''
    mkdir -p check/pkgs/mykonos-wake check/tests/mykonos-wake
    cp ${./wake.py} check/pkgs/mykonos-wake/wake.py
    cp ${../../tests/mykonos-wake/test_wake.py} check/tests/mykonos-wake/test_wake.py
    ${python3}/bin/python3 check/tests/mykonos-wake/test_wake.py
    ...
    makeWrapper ${python}/bin/python3 $out/bin/mykonos-wake \
    ...
    makeWrapper ${python}/bin/python3 $out/bin/mykonos-dictate \
      --prefix PATH : ${
        lib.makeBinPath [
          pipewire
          openssh
