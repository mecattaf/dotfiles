...
| [`runbook-conventions.md`](runbook-conventions.md) | **How a runbook suppresses a unit on NixOS** — why a plain mask fails on the read-only `/etc/systemd/system` store symlink, and the intent → wrong → right table (`mask --runtime`, `enable = false` + rebuild, move the data, stop the timer). |
