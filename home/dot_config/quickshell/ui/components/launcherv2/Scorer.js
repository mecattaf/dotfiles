...
function score(item, query, frecencyData) {
    ...
    if (textScore === 0 && item.keywords) {
        for (var i = 0; i < item.keywords.length; i++) {
            ...
            if (keywordScore > 0) { textScore = keywordScore * 0.3; break }
        }
    }
...
function scoreItems(items, query, getFrecencyFn) {
    ...
    for (var i = 0; i < items.length; i++) {
        ...
        if (itemScore > 0 || !query || query.length === 0) {
            scored.push({ item: item, score: itemScore })
        }
    }
...
function groupBySection(scoredItems, sectionOrder, sortAlphabetically, maxPerSection) {
    ...
    for (var i = 0; i < sectionOrder.length; i++) {
        ...
        sections[sectionId] = {
            id: sectionId, title: sectionOrder[i].title, icon: sectionOrder[i].icon,
            priority: sectionOrder[i].priority, items: [], collapsed: false, flatStartIndex: 0
        }
    }
    ...
    for (var i = 0; i < sectionOrder.length; i++) {
        ...
        if (section && section.items.length > 0) {
            if (sortAlphabetically && section.id === "apps") {
                section.items.sort(function (a, b) { return (a.name || "").localeCompare(b.name || "") })
            }
            result.push(section)
        }
    }
...
function flattenSections(sections) {
    ...
    for (var i = 0; i < sections.length; i++) {
        ...
        flat.push({ isHeader: true, section: section, sectionId: section.id, sectionIndex: i })
        var itemStart = flat.length
        section.flatStartIndex = itemStart
        if (!section.collapsed) {
            for (var j = 0; j < section.items.length; j++) {
                flat.push({
                    isHeader: false, item: section.items[j], sectionId: section.id,
                    sectionIndex: i, indexInSection: j
                })
            }
        }
        ...
        if (itemCount > 0) {
            bounds[section.id] = { start: itemStart, end: itemEnd, count: itemCount }
        }
    }
