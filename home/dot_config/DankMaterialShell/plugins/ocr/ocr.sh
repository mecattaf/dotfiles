...
if [[ -n "$ocr_text" ]]; then
    ...
    $NOTIFY && notify-send -a "DankMaterialShell" "OCR Success" "Text copied to clipboard"
else
    $NOTIFY && notify-send -a "DankMaterialShell" "OCR Failed" "No text recognized or operation failed"
fi
