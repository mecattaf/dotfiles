...
<!-- Vendored from cathrynlavery/diagram-design (MIT). Local grafts appended at the end — see "Local additions". -->
...
## Local additions (grafts)

These references were grafted into this vendored skill locally — they are not part of the
upstream cathrynlavery/diagram-design corpus. Load them on demand like the other references.

| Graft | Reference | Source |
|---|---|---|
| **Architecture — dark theme** — dark, high-contrast architecture recipe (cyan/emerald/violet component palette, JetBrains Mono, slate-950 grid ground, arrow z-order + masking + spacing tricks). An alternative skin for the `Architecture` type when a dark ground is wanted. | [type-architecture-dark.md](references/type-architecture-dark.md) — template `assets/template-architecture-dark.html`, examples `assets/example-architecture-dark-*.html` | Cocoon AI architecture-diagram-generator (MIT) |
| **Gantt / project timeline** — a 15th diagram type not in the upstream selection table: tasks with start+duration+dependencies on a time axis. | [type-gantt.md](references/type-gantt.md) | figma-plugin-jun2026 figma-generate-diagram (thinking only) |
| **Context gathering** — gather real ground truth before drawing; don't hallucinate edges. Applies to every type. | [figma-context-gathering.md](references/figma-context-gathering.md) | figma-plugin-jun2026 figma-generate-diagram (thinking only) |
