...
let
  ...
  configDirs = [ "niri" "kitty" "fish" "starship" "zathura" "yt-dlp" "kanshi" "qt6ct" "shpool" ];

  # Python interpreter backing the niri helper bin/ scripts (wifi-menu, fzf-nmcli, …).
  pythonForNiri = pkgs.python3.withPackages (ps: with ps; [
    pycairo pygobject3 pillow psutil pywayland requests setproctitle watchdog numpy ijson
  ]);
...
in
{
  ...
  # ---------------------------------------------------------------------------
  xdg.configFile = lib.genAttrs configDirs (d: { source = link "dot_config/${d}"; });
  ...
  gtk = {
    enable = true;
    theme = { name = "MacTahoe-grey-dark"; package = pkgs.mactahoe-gtk-theme; };
    iconTheme = { name = "MacTahoe-grey-dark"; package = pkgs.mactahoe-icon-theme; };
  ...
  home.packages = with pkgs; [
    ...
    # fish init + shell
    eza zoxide atuin starship fzf bat ripgrep fd jq yq-go glow

    # niri / wayland desktop tooling + D2 native re-point (brightnessctl/playerctl/
    # swaybg; NO rofi by decision)
    brightnessctl playerctl swaybg wl-clipboard cliphist wl-gammarelay-rs kanshi
    grim slurp wf-recorder wl-mirror wmctrl wtype lisgd ddcutil cava
    pamixer pavucontrol nwg-look

    # the python interpreter the niri helper scripts need
    pythonForNiri

    # media / viewers
    yt-dlp aria2 mpv imv vlc zathura ffmpeg-full ffmpegthumbnailer

    # files / nautilus + open-any-terminal (decided keep)
    nautilus nautilus-open-any-terminal xdg-terminal-exec

    # session persistence + terminal
    shpool kitty

    # agent / dev tooling (pi.nix + llm-agents.nix come in the agent session)
    gh google-cloud-sdk cloudflared
  ...
  #   - nvim → task #6 (nvim-sweep.md: programs.neovim + lazy-nix-helper + mason→Nix)
