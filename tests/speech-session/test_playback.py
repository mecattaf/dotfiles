...
spec=importlib.util.spec_from_file_location('p',Path(__file__).parents[2]/'pkgs/speech-wake/playback.py');p=importlib.util.module_from_spec(spec);spec.loader.exec_module(p)
class Tests(unittest.TestCase):
 def test_unknown_fails_closed_and_cue_exempt(self):
  ...
  s.update([{'id':1,'info':{'state':'running','props':{'media.class':'Stream/Output/Audio','node.name':'speech-listening-cue'}}}]);self.assertIsNone(s.reason())
