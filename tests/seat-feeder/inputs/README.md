...
sources pointed here. The 60-tick replay pins its clock with
`TALLY_FEEDER_NOW`; publication-order checks deliberately use the wall clock.
Nothing in the oracle touches a credential, a network socket, or a live
rollout.
...
For D-B54, the Claude fixture optionally sleeps by seat when
`TALLY_FIXTURE_READER_DELAYS` contains a JSON map. One check scales three equal
delays against the service cap to distinguish concurrent from sequential
reads. A second staggers them so batching is visible from the row files'
arrival times. The variable is unset in ordinary replay invocations.
